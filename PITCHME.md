...
### レンダリングエンジンとしてWebkit系・Blink系・Gecko系・Trident系・Edge HTMLがある

+++

### それぞれでHTML・CSS・JavaScriptを実行して返ってくる結果が異なる場合がある

+++

### 特に新しい技術は各ブラウザで実装タイミングがことなるため、使えないブラウザもある

+++

### 異なるブラウザ間においても同一の結果、もしくはブラウザごとに適切な結果になるようコーディングすることをクロスブラウザ対応と呼ぶ

+++

### ブラウザ間の差を吸収するためにCSSハックやベンダープレフィックス、JavaScriptでユーザーエージェントを見て振る舞いを変える方法などを用いる
...
1. フォントは各ブラウザにインストールされていないと使えない
2. Webフォントという技術ですべての環境で同じフォントを使うことができる
3. Webフォントアイコンと呼ばれるアイコンをフォント化して手軽に扱えるようにする手法がある

参考：[Google Fontsの使い方](https://saruwakakun.com/html-css/basic/google-fonts)
...
### 課題１
オンライン学習サービスのProgateで「HTML & CSS - 学習コース 初級編 -」を一通り行う
